...
class CinemaTestCase(APITestCase):
    def setUp(self):
        ...
        kina=("pod Baranami", "Duże", "Małe", "Max", "Czerwone", "Kameralne", "Multi", "Helios", "City", "Uciecha", "Rodzinne", "Gloria" )
        self.faker = Faker("pl_PL")
        for _ in range(5):
            Cinema.objects.create(name="Kino {}".format(choice(kina)))
        for _ in range(3):
            self._create_fake_movie()
    def _random_cinema(self):
        """Return a random cinema object from db."""
        cinemas = Cinema.objects.all()
        return cinemas[randint(0, len(people) - 1)]
